<div align="center">
  ...
  <p>
    <a href="https://centrio.me"><b>🌐 Website</b></a>
    &nbsp;·&nbsp;
    <a href="https://centrio.me/download"><b>📥 Download</b></a>
    &nbsp;·&nbsp;
...
[Why Centrio](#why-centrio) · [Features](#features) · [Pro / Team](#pro--team) · [Download](#download) · [What's New](#whats-new--v260) · [Building from Source](#building-from-source) · [CI/CD](#cicd) · [Tech Stack](#tech-stack) · [Architecture](#architecture) · [License](#license)
...
### 🗂️ 100+ Services, One Sidebar

Telegram · WhatsApp · Discord · VK · Slack · Notion · Gmail · Signal · Zoom · WeChat · LINE · Figma · Jira · MAX · and any URL you add manually.

### 🤖 AI Assistant *(Pro/Team)*

Ask it in plain language and it just does it: switches tabs, checks unread counts, reads your notification bell, controls the built-in VPN, drives the mini media player, and manages your Notes — creating, pinning, and deleting entries on request.

### 📝 Notes *(Pro/Team)*

A real notes plugin, not an afterthought: plain notes and shopping lists, synced across every device.

- 🔍 Full-text search across notes and checklist items
- 🎨 Six color labels for quick visual sorting
- 🗄️ Archive — get a note out of the way without deleting it
- ↕️ Drag-and-drop reordering
- 📌 Pinning, 📋 duplication, and a last-edited date on every card
...
### 🧩 Chrome Extensions *(Pro/Team)*

Install AdBlock, Grammarly, Translate and other extensions directly inside Centrio — no separate browser needed.
...
| 🎨 **Premium Settings** | A freshly redesigned settings window |
| 📁 **Folders** *(Pro/Team)* | Group services into folders — Work, Personal, Projects |
| ☁️ **Cloud sync** *(Pro/Team)* | Settings, messengers and folders synced across devices |
...
### v2.3.0
- Fixed login via Google/Yandex/etc. inside messengers sometimes opening in an external browser and never returning
- Renamed "Yandex Chat" → "Я.Мессенджер", updated its address
- Fixed clipboard/mic/camera permissions silently failing in some messengers
- Fixed duplicate notifications in split-screen mode
- Fixed usage-time tracking only counting the primary split-screen pane

### v2.2.0
- New: a second, right-hand sidebar — Assistant (soon), Todos and notifications moved there
- New: Todos — separate lists (categories), pin important tasks, mark done
- New: first launch now starts with sign-in (Google, Yandex or email) or guest mode — either way, 14 days of Pro free
- New: collapsible left sidebar with labels
- New: subscription status footer with a "Renew" button
- New "Embedded" theme, now the default
- Interface density (Compact / Normal / Comfortable) now actually resizes the sidebar and icons

### v2.1.0
- Fixed a monospace "AI" font leaking into the header and small titles on the site
- Screenshots on the site now open full-size on click (lightbox)
- New: a 14-day free Pro promo code popup on the site
- New: promo codes can now be redeemed directly in the app

### v2.0.0
- Major update: Russian is now the default app language
- Removed the onboarding tour — it did more harm than good
- Fixed the app hanging at 91% on startup

### v1.9.x
- Sidebar dividers, a downloads manager, popup/OAuth/VPN fixes
- Badging API for unread counts, DownloadURL-based drag-out
- Split-screen layouts (grid and side-by-side)

### v1.6.x
- **Translate selected text** — right-click any selection → Translate, instant overlay
- **Screenshot tool** — capture the current service window from the menu bar
- **Catalog overhaul** — categories, search, one-click add
- Built-in **AdBlock**, **Force Dark Mode**, Ctrl+K slash commands
- **Chrome Extension support** — install from a curated catalog

### v1.5.x
- macOS & Linux builds — full cross-platform CI/CD
- Language switching without an app relaunch
- VLESS + Reality + TCP/XHTTP support, subscription URL import
- 40+ messengers added
